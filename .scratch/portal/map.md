...
- [Antenna Models enumerated (16, tiered)](./issues/01-enumerate-antenna-models.md) — T1 closed-form staples (9, share λ×k engine) / T2 published-design (3) / T3 complex (4); short antennas = base model + Loading Coil handoff (¼-wave Vertical is the handoff home). [List](./assets/antenna-models-list.md). **Tier 1 + Tier 2 BUILT ✅** — Tier 1: shared closed-form engine (`src/lib/antennas/engine.ts`, length = factor·λ·k) + data specs (`models.ts`) + `Designer.svelte` (dipole/vertical/loop/wire SVG); loadable models deep-link "Shorten & load → Loading Coil". Tier 2: `tier2.ts` (Yagi-Uda N-element parametric, Moxon rectangle Cebik, J-Pole/Slim-Jim variant) + `Tier2Designer.svelte` (yagi/moxon/jpole SVG), published-design + Accuracy Note. Both served via `/antennas/[slug]`. 14 antenna engine tests. **Only Tier 3 (4 models) still render the stub.**
